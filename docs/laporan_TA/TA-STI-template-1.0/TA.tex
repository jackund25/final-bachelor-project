...
% Ruang regang darurat pada lintasan terakhir penyusunan paragraf. Dipakai agar
% paragraf yang memuat nama kode panjang tidak meluber keluar batas kolom teks.
\emergencystretch=3em
\usepackage{floatrow} % for customizing float (figure and table) positions
\usepackage{array} % for better arrays (eg. in tabular environment)
\usepackage{threeparttable} % for tables with notes
\usepackage{tabularx} % for tables with adjustable-width columns
% Kolom lentur untuk tabel yang kolomnya memuat kalimat. Dipakai agar tabel lebar
% membungkus di dalam \textwidth alih-alih meluber keluar margin.
\newcolumntype{L}{>{\raggedright\arraybackslash}X}
\newcolumntype{R}{>{\raggedleft\arraybackslash}X}
% Gaya paragraf catatan di bawah tabel. \centering dari lingkungan table
% menular ke paragraf catatan sehingga barisnya ikut rata tengah; makro ini
% memulihkan rata kiri-kanan seperti paragraf biasa.
\newcommand{\tabnotestyle}{\footnotesize\leftskip=0pt\rightskip=0pt\parfillskip=0pt plus 1fil\noindent}
\usepackage{rotating} % for sidewaysfigure: landscape pages for wide diagrams
...
% --- biblatex tidak memiliki berkas pelokalan untuk bahasa Indonesia, sehingga
% ia memperingatkan "Language 'indonesian' not supported" lalu meminta rerun
% pada tiap kompilasi tanpa pernah tuntas. Pemetaan ini menyuruhnya memakai
% cms-american.lbx (pelokalan biblatex-chicago) bagi bahasa indonesian.
% Bahasa yang benar-benar dipakai saat mencetak daftar pustaka ditentukan
% opsi language=english di atas, yang oleh biblatex/biblatex-chicago
% diselesaikan menjadi 'american' (lihat TA.log: "Trying to load language
% 'american' -> 'cms-american'"). Karena \DefineBibliographyStrings hanya
% berlaku bagi nama bahasa yang persis sama, terjemahan di bawah didaftarkan
% untuk ketiga nama sekaligus agar tidak bergantung pada nama mana yang aktif. ---
\DeclareLanguageMapping{indonesian}{cms-american}
...
% --- Cegah baris sisa dan kata ke-hyphenate persis di batas
% halaman. Ini murni aturan line-breaking LaTeX, tidak mengubah format
% template (margin/font/judul/penomoran tetap sama). ---
\widowpenalty=10000
\clubpenalty=10000
\brokenpenalty=10000
...
% --- Biarkan halaman berakhir di tempat teksnya habis. Tanpa ini kelas book
% dalam mode twoside memakai \flushbottom, yang memaksa tiap halaman
% setinggi persis sama dengan cara meregangkan \parskip yang elastis,
% sehingga jarak antar-paragraf melebar tidak konsisten antar-halaman. ---
\raggedbottom
...
\input{5 Abstrak.tex}
%\input{5a Abstrak_eng.tex} % Abstrak bahasa Inggris untuk berkas yudisium/digilib; beri tanda % bila tidak dipakai.
